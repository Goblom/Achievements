package codes.goblom.achievements.misc;
...
// ********************************
// This messes with the config. It removes all comments and special data
// ********************************
//        FileConfiguration config = plugin.getConfig();
//                          config.addDefault("mysql.host", "localhost");
//                          config.addDefault("mysql.port", "3306");
//                          config.addDefault("mysql.database", "achievements");
//                          config.addDefault("mysql.username", "root");
//                          config.addDefault("mysql.password", "");
//                          config.addDefault("mysql.refresh", false);
//                          config.options().copyDefaults(true);
//
//        plugin.saveConfig();
    }

    public void start(String database, List<Class<?>> databaseClasses) {
        FileConfiguration config = plugin.getConfig();
        String driver = "com.mysql.jdbc.Driver";
        int port = config.getInt("mysql.port", 3306);
        String url = "jdbc:mysql://" + config.getString("mysql.host") + ":" + port + "/" + database;
        String username = config.getString("mysql.username", "root");
        String password = config.getString("mysql.password", "");
        String isolation = "TRANSACTION_SERIALIZABLE"; //config.getString("mysql.isolation");
        boolean logging = false;
        boolean rebuild = config.getBoolean("mysql.refresh", false);
...
//            ebeanServer.getServerCacheManager().setCaching(clazz, false);
            try {
				// Do a simple query which only throws an exception if the table
                // does not exist
                ebeanServer.find(clazz).findRowCount();
